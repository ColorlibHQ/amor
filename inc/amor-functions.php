...
=========================================================*/
function amor_opt( $id = null, $default = '' ) {
...
function amor_site_icon(){
	if ( ! has_site_icon() ) {
		$html = '';
		$icon_path = AMOR_DIR_ASSETS_URI . 'img/favicon.png';
...
=========================================================*/
function amor_meta( $id = '' ){
    
    $value = get_post_meta( get_the_ID(), '_amor_'.$id, true );
...
/*=========================================================
	Blog Date Permalink
=========================================================*/
function amor_blog_date_permalink(){
...
/*========================================================
	Blog Excerpt Length
========================================================*/
if ( ! function_exists( 'amor_excerpt_length' ) ) {
	function amor_excerpt_length( $limit = 30 ) {
...
==========================================================*/
if ( ! function_exists( 'amor_posted_comments' ) ) {
    function amor_posted_comments(){
        
        $comments_num = get_comments_number();
        if( comments_open() ){
            if( $comments_num == 0 ){
                $comments = esc_html__(' No Comments','amor');
            } elseif ( $comments_num > 1 ){
                $comments= $comments_num . esc_html__(' Comments','amor');
            } else {
                $comments = esc_html__( ' 1 Comment','amor' );
            }
            $comments = '<i class="ti-comments"></i> '. $comments;
        } else {
            $comments = esc_html__( ' Comments are closed', 'amor' );
        }
...
/*===================================================
	Post embedded media
===================================================*/
function amor_embedded_media( $type = array() ){
...
/*===================================================
	WP post link pages
====================================================*/
function amor_link_pages(){
    wp_link_pages( array(
    'before'      => '<div class="page-links"><span class="page-links-title">' . esc_html__( 'Pages:', 'amor' ) . '</span>',
    'after'       => '</div>',
    'link_before' => '<span>',
    'link_after'  => '</span>',
    'pagelink'    => '<span class="screen-reader-text">' . esc_html__( 'Page', 'amor' ) . ' </span>%',
...
/*====================================================
	Theme logo
====================================================*/
function amor_theme_logo( $class = '' ) {

    $siteUrl = home_url('/');
    // site logo
		
	$custom_logo_id = get_theme_mod( 'custom_logo' );
	$imageUrl = wp_get_attachment_image_src( $custom_logo_id , 'amor_logo_126x34' );
	
	if( !empty( $imageUrl[0] ) ){
		$siteLogo = '<a class="'.esc_attr( $class ).'" href="'.esc_url( $siteUrl ).'"><img src="'.esc_url( $imageUrl[0] ).'" alt="amor logo"></a>';
	}else{
	...
	if ( ! is_page_template( 'template-builder.php' ) ) {
        ...
        <section class="hero-banner d-flex align-items-center">
            <div class="container">
				...
				<?php
				if ( is_category() ) {
					single_cat_title( __('Category: ', 'amor') );
					...
					echo esc_html__( 'Search for: ', 'amor' ) . get_search_query();

				} elseif ( ! ( is_404() ) && ( is_single() ) || ( is_page() ) ) {
					echo  get_the_title();

				} elseif ( is_home() ) {
					echo esc_html__( 'Blog', 'amor' );

				} elseif ( is_404() ) {
					echo esc_html__( '404 error', 'amor' );
...
=================================================*/
function amor_pull_right( $id = '', $condation ){
...
/*======================================================
	Inline Background
======================================================*/
function amor_inline_bg_img( $bgUrl ){
    $bg = '';
...
/*======================================================
	Blog Category
======================================================*/
function amor_featured_post_cat(){
...
========================================================*/
function amor_sidebar_opt(){

    $sidebarOpt = amor_opt( 'amor_blog_layout' );
    $sidebar = '1';
...
/**================================================
	Themify Icon
 =================================================*/
function amor_themify_icon(){
    return[
        'cap'     => __('Icon Cap', 'amor'),
        'bag'     => __('Icon Bag', 'amor'),
        'shirt'   => __('Icon T-shirt', 'amor'),
        'cafe'    => __('Icon Cafe', 'amor'),
    ];
}
...
============================================================*/
function amor_contact7_form_content( $template, $prop ) {
...
add_filter( 'wpcf7_default_template', 'amor_contact7_form_content', 10, 2 );
...
/*============================================================
	Pagination
=============================================================*/
function amor_blog_pagination(){
	echo '<nav class="blog-pagination justify-content-center d-flex">';
        the_posts_pagination(
            array(
                'mid_size'  => 2,
                'prev_text' => __( '<span class="ti-angle-left"></span>', 'amor' ),
                'next_text' => __( '<span class="ti-angle-right"></span>', 'amor' ),
                'screen_reader_text' => ' '
            )
        );
	echo '</nav>';
...
=============================================================*/
if( ! function_exists('amor_blog_single_post_navigation') ) {
	function amor_blog_single_post_navigation() {
				...
				<div class="row">
					<div class="col-lg-6 col-md-6 col-12 nav-left flex-row d-flex justify-content-start align-items-center">
						...
						if( get_next_post_link() ){
							...
							if( has_post_thumbnail() ){
								...
								<div class="thumb">
									<a href="<?php the_permalink(  absint( $nextPost->ID )  ) ?>">
										<?php echo get_the_post_thumbnail( absint( $nextPost->ID ), 'amor_np_thumb', array( 'class' => 'img-fluid' ) ) ?>
									</a>
								</div>
								<?php
							} ?>
							...
							<div class="detials">
								<p><?php echo esc_html__( 'Prev Post', 'amor' ); ?></p>
								<a href="<?php the_permalink(  absint( $nextPost->ID )  ) ?>">
					...
					<div class="col-lg-6 col-md-6 col-12 nav-right flex-row d-flex justify-content-end align-items-center">
						...
						if( get_previous_post_link() ){
							...
							?>
							<div class="detials">
								<p><?php echo esc_html__( 'Next Post', 'amor' ); ?></p>
							...
							<div class="thumb">
								<a href="<?php the_permalink(  absint( $prevPost->ID )  ) ?>">
									<?php echo get_the_post_thumbnail( absint( $prevPost->ID ), 'amor_np_thumb', array( 'class' => 'img-fluid' ) ) ?>
								</a>
							</div>
							<?php
						} ?>
...
/*===================================================
 Amor Comment Template Callback
 ====================================================*/
function amor_comment_callback( $comment, $args, $depth ) {
				...
				<div class="desc">
					...
					<div class="d-flex justify-content-between">
						<div class="d-flex align-items-center">
							<h5 class="comment_author"><?php printf( __( '<span class="comment-author-name">%s</span> ', 'amor' ), get_comment_author_link() ); ?></h5>
							<p class="date"><?php printf( __('%1$s at %2$s', 'amor'), get_comment_date(),  get_comment_time() ); ?><?php edit_comment_link( esc_html__( '(Edit)', 'amor' ), '  ', '' ); ?> </p>
							<?php if ( $comment->comment_approved == '0' ) : ?>
								<em class="comment-awaiting-moderation"><?php esc_html_e( 'Your comment is awaiting moderation.', 'amor' ); ?></em>
								<br>
							<?php endif; ?>
						</div>
...
add_filter('comment_reply_link', 'amor_replace_reply_link_class');
function amor_replace_reply_link_class( $class ) {
...
===========================================================*/
function amor_latest_blog( $post_cat = 'inspiration', $post_order = 'DESC', $excerpt_limit = 18 ){
		...
		<div class="col-lg-4 col-sm-6">
			<div class="single_blog">
				<div class="appartment_img">
					<?php
						if( has_post_thumbnail() ){
							the_post_thumbnail( 'amor_home_blog_350x374', ['alt' => get_the_title() ] );
						}
					?>
				</div>
				<div class="single_appartment_content">
					<a href="<?php the_permalink(); ?>"><h4><?php the_title(); ?></h4></a>
					<p><?php echo amor_excerpt_length( $excerpt_limit )?></p>
					<ul class="list-unstyled">
						<li><a href="<?php echo esc_url( amor_blog_date_permalink() )?>"> <span class="flaticon-calendar"></span> <?php the_time('M j, Y') ?> </a></li>
						<li><a href="<?php the_permalink(); ?>">  <?php echo amor_posted_comments();?> </a></li>
					</ul>
				</div>
			</div>
		</div>
...
/*=========================================================
    Share Button Code
===========================================================*/
function amor_social_sharing_buttons( $ulClass = '', $tagLine = '' ) {
